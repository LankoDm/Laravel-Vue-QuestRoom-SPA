...
const approveReview = async (id) => {
  try {
    ...
    if (review) {
      ...
      toast.success('Успішне одобрення відгуку');
    }
  } catch (error) {
...
<template>
  <div>
    ...
    <div v-else>
      ...
      <div v-for="review in paginatedReviews" :key="review.id"
        ...
        <div class="flex gap-1 mb-4 text-yellow-400">
          <svg v-for="i in 5" :key="i" class="w-5 h-5"
               :class="i <= review.rating ? 'fill-current' : 'text-gray-200 fill-current'" viewBox="0 0 20 20">
            <path
      ...
      </div> </div> <div v-if="totalPages > 1"
                         class="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-secondary mt-8">
        <div class="text-sm font-medium text-gray-500">
          Показано <span class="font-bold text-text">{{ (currentPage - 1) * itemsPerPage + 1 }}</span> -
          <span class="font-bold text-text">{{ Math.min(currentPage * itemsPerPage, filteredReviews.length) }}</span>
          із <span class="font-bold text-text">{{ filteredReviews.length }}</span>
        </div>
