...
const roomsChartPeriod = ref('all'); // Timeframe filter for the Doughnut chart ('all', '30', '7')
...
/**
 * Trigger the generation and download of the PDF report.
 */
...
/**
 * Computed configuration for the Revenue Bar Chart.
 */
...
/**
 * Computed configuration for the Rooms Popularity Doughnut Chart.
 * Reactively responds to changes in roomsChartPeriod.
 */
const roomsChartData = computed(() => {
    if (!stats.value || !stats.value.rooms_chart) return {labels: [], datasets: []};

    // Select data based on the current timeframe filter
    const periodData = stats.value.rooms_chart[roomsChartPeriod.value] || [];

    return {
        labels: periodData.map(item => item.name),
        datasets: [{
    ...
    <div>
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            ...
            <button @click="downloadPdf" :disabled="isDownloading"
                    class="flex items-center gap-2 bg-white text-primary font-bold px-4 py-2.5 rounded-xl border border-secondary shadow-sm hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50">
        ...
        <div v-else class="space-y-8 animate-fade-in">
          ...
          <span class="text-gray-500 font-medium mb-2 flex items-center gap-2">
            ...
            Всього відгуків
                ...
                <div
                    class="bg-white p-6 rounded-3xl shadow-sm border border-secondary flex flex-col justify-center transition-transform hover:-translate-y-1">
          <span class="text-gray-500 font-medium mb-1 flex items-center gap-2">
            <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path
                stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            Дохід за 7 днів
                    ...
                    <span class="text-3xl font-black text-emerald-600 mb-1">{{ formatPrice(stats.revenue_week) }}</span>
                    <span
                        class="text-[10px] text-gray-400 uppercase tracking-widest font-bold">за датою оформлення</span>
                </div>
                <div
                    class="bg-white p-6 rounded-3xl shadow-sm border border-secondary flex flex-col justify-center transition-transform hover:-translate-y-1">
          <span class="text-gray-500 font-medium mb-1 flex items-center gap-2">
            ...
            Дохід за 30 днів
                    ...
                    <span class="text-3xl font-black text-emerald-600 mb-1">{{
          ...
          <span class="text-gray-500 font-medium mb-2 flex items-center gap-2">
            ...
            <div v-if="stats.insights" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div
                    class="bg-gradient-to-br from-green-50 to-white p-5 rounded-2xl border border-green-100 shadow-sm transition-transform hover:-translate-y-1">
                    <span
                        class="text-xs font-bold text-green-600 uppercase tracking-wider block mb-1">Топ бронювань</span>
            ...
            {{ stats.insights.most_booked?.name || 'Немає даних' }}
                    ...
                    <span class="text-sm text-gray-500 font-medium">{{
                            stats.insights.most_booked?.bookings_count || 0
                        }} ігор</span>
                </div>
                <div
                    class="bg-gradient-to-br from-red-50 to-white p-5 rounded-2xl border border-red-100 shadow-sm transition-transform hover:-translate-y-1">
                    <span
                        class="text-xs font-bold text-red-500 uppercase tracking-wider block mb-1">Найменше бронювань</span>
                    <span class="font-black text-text text-lg block truncate"
                          :title="stats.insights.least_booked?.name">
            {{ stats.insights.least_booked?.name || 'Немає даних' }}
          </span>
                    <span class="text-sm text-gray-500 font-medium">{{
                            stats.insights.least_booked?.bookings_count || 0
                        }} ігор</span>
                </div>
                <div
                    class="bg-gradient-to-br from-yellow-50 to-white p-5 rounded-2xl border border-yellow-100 shadow-sm transition-transform hover:-translate-y-1">
            ...
            {{ stats.insights.best_rated?.name || 'Немає даних' }}
                    ...
                    <span class="text-sm text-gray-500 font-medium">{{
                            stats.insights.best_rated?.reviews_avg_rating ? Number(stats.insights.best_rated.reviews_avg_rating).toFixed(1) : 0
                        }} / 5</span>
                </div>
                <div
                    class="bg-gradient-to-br from-gray-50 to-white p-5 rounded-2xl border border-gray-200 shadow-sm transition-transform hover:-translate-y-1">
                    <span
                        class="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Найнижчий рейтинг</span>
            ...
            {{ stats.insights.worst_rated?.name || 'Немає даних' }}
                    ...
                    <span class="text-sm text-gray-500 font-medium">{{
                            stats.insights.worst_rated?.reviews_avg_rating ? Number(stats.insights.worst_rated.reviews_avg_rating).toFixed(1) : 0
                        }} / 5</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-secondary lg:col-span-1">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-bold text-text">Популярність</h3>

                        <select v-model="roomsChartPeriod"
                                class="bg-gray-50 border border-secondary text-gray-700 text-sm font-bold rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-primary cursor-pointer transition-all">
