...
const {parseImagesArray} = useRoomHelpers();
...
const form = ref({
    name: '', slug: '', description: '', difficulty: 'medium', age: '12+',
    hint_phrase: '', genre: '', min_players: 2, max_players: 4,
    weekday_price: null, weekend_price: null, duration_minutes: 60, is_active: 1
});

/**
 * Handle file input and generate local previews.
 */
...
/**
 * Save or update the room via FormData.
 */
const saveRoom = async () => {
    ...
    try {
        ...
        if (imageFiles.value.length > 0) {
            imageFiles.value.forEach(file => formData.append('image_path[]', file));
        }
    ...
    <div class="max-w-4xl mx-auto py-8">
        ...
        <div class="flex items-center gap-4 mb-8">
            <RouterLink :to="{ name: 'admin.rooms' }"
                        class="p-2 bg-white rounded-xl shadow-sm border border-secondary text-gray-500 hover:text-primary transition-colors">
        ...
        <form v-else @submit.prevent="saveRoom" class="space-y-8">
            <div v-if="errorMessage"
                 class="p-4 bg-red-50 text-red-600 rounded-xl border border-red-200 font-bold flex items-center gap-2 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="md:col-span-2 space-y-6 bg-white p-8 rounded-3xl shadow-sm border border-secondary">
                    ...
                    <div>
                        ...
                        <span v-if="validationErrors.name"
                              class="text-xs text-red-500 font-bold mt-1 block">{{ validationErrors.name[0] }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <span v-if="validationErrors.slug"
                              class="text-xs text-red-500 font-bold mt-1 block">{{ validationErrors.slug[0] }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <span v-if="validationErrors.description" class="text-xs text-red-500 font-bold mt-1 block">{{
                                validationErrors.description[0]
                            }}</span>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Секретна фраза-підказка</label>
                        <input v-model="form.hint_phrase" type="text"
                               placeholder="Наприклад: 'Ключ знаходиться під килимком'"
                               class="w-full px-4 py-3 rounded-xl border focus:outline-none transition-colors"
                        ...
                        <span v-if="validationErrors.hint_phrase" class="text-xs text-red-500 font-bold mt-1 block">{{
                                validationErrors.hint_phrase[0]
                            }}</span>
                    </div>
                    <div class="pt-4 border-t border-secondary">
                        ...
                        <span v-if="validationErrors.image_path" class="text-xs text-red-500 font-bold mt-1 mb-2 block">{{
                                validationErrors.image_path[0]
                            }}</span>
                        <div v-if="imagePreviews.length > 0" class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            <div v-for="(img, idx) in imagePreviews" :key="idx"
                                 class="w-full h-24 rounded-xl overflow-hidden border border-secondary bg-gray-100 shadow-sm">
                                <img :src="img" class="w-full h-full object-cover"/>
                            </div>
                        </div>
                        <p v-if="isEditMode && imageFiles.length === 0 && imagePreviews.length > 0"
                ...
                <div class="space-y-6 bg-white p-8 rounded-3xl shadow-sm border border-secondary h-fit">
                    ...
                    <div>
                        ...
                        <span v-if="validationErrors.age"
                              class="text-xs text-red-500 font-bold mt-1 block">{{ validationErrors.age[0] }}</span>
                    </div>
                    ...
                    <div>
                        ...
                        <span v-if="validationErrors.genre"
                              class="text-xs text-red-500 font-bold mt-1 block">{{ validationErrors.genre[0] }}</span>
                    </div>
                    <span v-if="validationErrors.min_players || validationErrors.max_players"
                          class="text-xs text-red-500 font-bold block">Перевірте кількість гравців</span>
                    <div>
                    ...
                    <div class="pt-4 border-t border-secondary">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input v-model="form.weekday_price" type="number" min="0" required
                            ...
                            <div>
                                ...
                                <input v-model="form.weekend_price" type="number" min="0" required
                        ...
                        <span v-if="validationErrors.weekday_price || validationErrors.weekend_price"
                              class="text-xs text-red-500 font-bold mt-2 block">Перевірте ціну</span>
                    </div>
                    ...
                    <div class="pt-4 border-t border-secondary flex items-center justify-between">
                        ...
                        <input v-model="form.is_active" type="checkbox" :true-value="1" :false-value="0"
                               class="w-5 h-5 text-primary rounded focus:ring-primary cursor-pointer border-secondary">
                    </div>
