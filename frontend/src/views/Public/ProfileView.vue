...
const historyTotalPages = computed(() => Math.ceil(pastBookings.value.length / historyPerPage) || 1);
const paginatedHistory = computed(() => {
  const start = (historyPage.value - 1) * historyPerPage;
  return pastBookings.value.slice(start, start + historyPerPage);
});
          ...
          <section v-if="pastBookings.length > 0">
            ...
            <div class="space-y-4">
              ...
              <div v-if="historyTotalPages > 1"
                ...
                <span class="text-sm font-bold text-gray-500">{{ historyPage }} / {{ historyTotalPages }}</span>
