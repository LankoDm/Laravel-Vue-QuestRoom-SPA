...
<template>
  <div>
    <div class="flex justify-between items-center mb-8">
      ...
      <button @click="fetchReviews" class="p-3 bg-white text-primary shadow-sm border border-secondary hover:bg-secondary rounded-xl transition-colors shrink-0">
    ...
    <div v-else-if="reviews.length === 0" class="bg-white p-12 rounded-3xl text-center text-gray-500 shadow-sm border border-secondary">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div v-for="review in reviews" :key="review.id"
        ...
        <div class="flex justify-between items-start mb-4">
          <div>
            <div class="font-bold text-text text-lg">{{ review.user?.name || 'Невідомий користувач' }}</div>
            <div class="text-xs text-primary font-bold mt-1">Кімната: {{ review.room?.name || 'ID ' + review.room_id }}</div>
          </div>
          <span v-if="!review.is_approved" class="bg-yellow-100 text-yellow-700 text-xs font-black px-2 py-1 rounded-lg">Новий</span>
        ...
        <div class="flex gap-1 mb-4 text-yellow-400">
          <svg v-for="i in 5" :key="i" class="w-5 h-5" :class="i <= review.rating ? 'fill-current' : 'text-gray-200 fill-current'" viewBox="0 0 20 20">
        ...
        <div class="flex items-center justify-between pt-4 border-t border-secondary mt-auto">
          <span class="text-xs text-gray-400 font-medium">{{ formatDate(review.created_at) }}</span>

          <div class="flex gap-2">
            <button v-if="!review.is_approved" @click="approveReview(review.id)" title="Опублікувати на сайті"
