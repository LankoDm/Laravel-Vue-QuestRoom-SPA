...
const fetchRooms = async (page = 1) => {
  ...
  try {
    ...
    if (filters.value.difficulty && filters.value.difficulty.length > 0) params.difficulty = filters.value.difficulty;
    if (filters.value.age && filters.value.age.length > 0) params.age = filters.value.age;
    if (filters.value.genres && filters.value.genres.length > 0) params.genres = filters.value.genres;
    ...
    <div class="py-16 px-6 max-w-7xl mx-auto">
      ...
      <div class="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div class="w-full lg:w-1/4 shrink-0 lg:sticky lg:top-24">
          <div class="bg-white p-6 rounded-3xl shadow-sm border border-secondary space-y-6">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-4">
              ...
              <button @click="resetFilters" v-if="filters.difficulty.length || filters.age.length || filters.genres.length || filters.search || filters.players_count"
            ...
            <div class="flex gap-4">
              <div class="flex-1">
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">К-ть людей</label>
                <input v-model="filters.players_count" @input="onInput" type="number" min="1" placeholder="Гравців"
                       class="w-full px-4 py-3 rounded-xl border border-secondary focus:ring-2 focus:ring-primary outline-none transition-colors bg-gray-50 text-text font-medium">
              </div>
              <div class="flex-1">
            ...
            <div class="border-t border-gray-100 pt-4">
              <button @click="toggleSection('difficulty')" type="button" class="w-full flex justify-between items-center cursor-pointer mb-2">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-wider cursor-pointer">Складність</label>
                <svg class="w-5 h-5 text-gray-400 transition-transform duration-300" :class="{'rotate-180': openSections.includes('difficulty')}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
              </button>

              <div v-show="openSections.includes('difficulty')" class="space-y-2 mt-3 animate-fade-in">
                <label v-for="(label, val) in {'easy': 'Легкий', 'medium': 'Середній', 'hard': 'Складний', 'ultra hard': 'Експерт'}" :key="val"
                       class="flex items-center group cursor-pointer">
                  <input type="checkbox" :value="val" v-model="filters.difficulty" @change="fetchRooms(1)"
                  ...
                  <span class="ml-3 text-sm font-medium text-gray-600 group-hover:text-primary transition-colors">{{ label }}</span>
                </label>
              </div>
            </div>

            <div class="border-t border-gray-100 pt-4">
              <button @click="toggleSection('age')" type="button" class="w-full flex justify-between items-center cursor-pointer mb-2">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-wider cursor-pointer">Вік</label>
                <svg class="w-5 h-5 text-gray-400 transition-transform duration-300" :class="{'rotate-180': openSections.includes('age')}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
              </button>

              <div v-show="openSections.includes('age')" class="grid grid-cols-2 gap-2 mt-3 animate-fade-in">
                <label v-for="val in ['0+', '8+', '10+', '12+', '16+', '18+']" :key="val"
                       class="flex items-center group cursor-pointer">
                  <input type="checkbox" :value="val" v-model="filters.age" @change="fetchRooms(1)"
                         class="w-5 h-5 text-primary rounded border-gray-300 focus:ring-primary cursor-pointer transition-colors bg-gray-50 checked:bg-primary">
                  <span class="ml-2 text-sm font-medium text-gray-600 group-hover:text-primary transition-colors">{{ val }}</span>
                </label>
              </div>
            </div>

            <div class="border-t border-gray-100 pt-4">
              <button @click="toggleSection('genres')" type="button" class="w-full flex justify-between items-center cursor-pointer mb-2">
                <label class="text-sm font-bold text-gray-700 uppercase tracking-wider cursor-pointer">Жанр</label>
                <svg class="w-5 h-5 text-gray-400 transition-transform duration-300" :class="{'rotate-180': openSections.includes('genres')}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
              </button>

              <div v-show="openSections.includes('genres')" class="space-y-2 mt-3 animate-fade-in">
                <label v-for="(label, val) in {'horror': 'Жахи', 'detective': 'Детектив', 'action': 'Екшн', 'mystic': 'Містика', 'adventure': 'Пригоди'}" :key="val"
                       class="flex items-center group cursor-pointer">
        ...
        <div class="w-full lg:w-3/4">
          ...
          <div v-if="isLoading" class="text-center text-xl text-primary animate-pulse py-20 font-bold bg-white rounded-3xl border border-secondary shadow-sm">
            Завантаження квестів
          </div>

          <div v-else-if="rooms.length === 0" class="text-center py-20 text-gray-500 bg-white border border-secondary rounded-3xl font-medium shadow-sm">
              ...
              <div class="p-6 flex flex-col flex-grow">
                ...
                <div class="flex flex-wrap gap-4 mb-4 text-sm font-bold text-gray-500">
                  ...
                  <div v-if="room.genre" class="flex items-center gap-1 truncate max-w-full">
                    <svg class="w-4 h-4 text-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"></path></svg>
